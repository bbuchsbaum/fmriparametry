...
vignette: '%\VignetteIndexEntry{Understanding Parametric HRF Estimation with Taylor
  Approximation} %\VignetteEngine{knitr::rmarkdown} %\VignetteEncoding{UTF-8}'
params:
  family: red
  preset: homage
css: albers.css
...
plot_hrf_variations <- function() {
  ...
  for (tau in c(4, 6, 8)) {
    hrf <- fmrihrf::hrf_lwu(t, tau = tau, sigma = 2.5, rho = 0.35)
  ...
  for (sigma in c(1.5, 2.5, 3.5)) {
    hrf <- fmrihrf::hrf_lwu(t, tau = 6, sigma = sigma, rho = 0.35)
  ...
  for (rho in c(0, 0.35, 0.7)) {
    hrf <- fmrihrf::hrf_lwu(t, tau = 6, sigma = 2.5, rho = rho)
...
demonstrate_taylor <- function() {
  ...
  # True HRF
  hrf_true <- fmrihrf::hrf_lwu(t, tau = theta_true[1], 
                               sigma = theta_true[2], 
                               rho = theta_true[3])
  
  # HRF at expansion point
  hrf_0 <- fmrihrf::hrf_lwu(t, tau = theta_0[1], 
                            sigma = theta_0[2], 
                            rho = theta_0[3])
  
  # Get Taylor basis (HRF and derivatives)
  basis <- fmrihrf::hrf_basis_lwu(theta0 = theta_0, t = t)
...
generate_bold <- function(events, params, noise_sd = 0.5) {
  ...
  hrf <- fmrihrf::hrf_lwu(t_hrf, tau = params[1], 
...
plot_fitted_hrfs <- function(true_params, estimated_params) {
  ...
  for (v in 1:3) {
    # True HRF
    hrf_true <- fmrihrf::hrf_lwu(t, tau = true_params[v,1], 
                                 sigma = true_params[v,2], 
                                 rho = true_params[v,3])
    
    # Estimated HRF
    hrf_est <- fmrihrf::hrf_lwu(t, tau = estimated_params[v,1], 
...
### 3. Multi-Seed Estimation

By default, the package tries multiple starting points spanning the HRF
parameter space and keeps the best fit per voxel. This is especially valuable
when true HRF shapes vary widely across the brain --- a single expansion point
can miss regions with early peaks or wide responses.

```{r multi-seed, eval=FALSE}
# Enabled by default (multi_seed = TRUE)
# The built-in grid covers early/late, narrow/wide, and low/high undershoot

# To supply your own seed grid (each row = one starting point):
fit_custom <- estimate_parametric_hrf(
  fmri_data = fmri_data,
  event_model = event_model,
  seed_grid = rbind(
    c(4, 1.5, 0.2),   # early, narrow
    c(6, 2.5, 0.4),   # canonical
    c(8, 3.5, 0.6)    # late, wide
  )
)
```
