...
#' Try expression with context and fallback
#'
#' @param expr Expression to try
#' @param context Context for error messages
#' @param fallback Fallback value or expression
        ...
        tryCatch(
          ...
          error = function(e2) {
            stop(sprintf("Error in %s: %s. Fallback also failed: %s", 
                        context, primary_error, e2$message), call. = FALSE)
          }
        )
      } else {
        stop(sprintf("Error in %s: %s", context, primary_error), call. = FALSE)
      }
    }
  )
}
...
.get_available_memory <- function() {
  ...
  if (.Platform$OS.type == "windows") {
    # Windows
    tryCatch(memory.size(max = TRUE) * 1024^2, error = function(e) 8e9)
  } else {
    # Unix-like systems
    tryCatch({
      ...
      if (file.exists("/proc/meminfo")) {
        ...
        if (length(available_line) > 0) {
          as.numeric(gsub("[^0-9]", "", available_line)) * 1024
        } else {
          8e9  # 8GB default
        }
      } else {
...
#' @param operation_name Name of operation for messages
#' @return Logical whether enough memory is available
#' @keywords internal
.check_memory_available <- function(required_bytes, operation_name = "operation") {
  available <- .get_available_memory()
  
  if (required_bytes > available * 0.8) {  # Use 80% threshold
    warning(sprintf(
      "Operation '%s' may require more memory than available: %.1f GB required, %.1f GB available",
      operation_name,
...
#' Execute with comprehensive error context
#'
#' @param expr Expression to evaluate
#' @param context Character description of context
#' @param fallback Optional fallback expression
#' @return Result of expression or fallback
#' @keywords internal
.try_with_context <- function(expr, context = "", fallback = NULL) {
  tryCatch(
    expr,
    error = function(e) {
...
#' Check memory requirements before operation
#'
#' @param required_bytes Estimated bytes needed
#' @param operation Character description of operation
#' @return Logical whether to proceed
#' @keywords internal
.check_memory_available <- function(required_bytes, operation = "") {
  # Get available memory (platform-specific)
  available <- .get_available_memory()
  
  if (required_bytes > available * 0.8) {  # 80% threshold
    warning(sprintf(
...
.get_available_memory <- function() {
  if (.Platform$OS.type == "windows") {
    memory.limit() * 1024^2  # Convert MB to bytes
  } else {
    # Unix-like systems
    tryCatch({
      as.numeric(system("awk '/MemAvailable/ {print $2}' /proc/meminfo", 
                       intern = TRUE)) * 1024  # KB to bytes
    }, error = function(e) {
